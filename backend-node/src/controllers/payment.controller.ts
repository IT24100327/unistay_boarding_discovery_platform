  ...
  try {
    ...
    const payment = await prisma.$transaction(async (tx) => {
      ...
      const confirmedTotal = rentalPeriod.payments
        .filter((p) => p.status === PaymentStatus.CONFIRMED)
        .reduce((sum, p) => sum.add(p.amount), new Decimal(0));
      const remaining = new Decimal(rentalPeriod.amountDue).sub(confirmedTotal);
...
// GET /api/v1/payments/my-boardings  (owner)
    ...
    const payments = await prisma.payment.findMany({
      ...
      select: {
        ...
        reservation: {
          select: {
            id: true,
            boarding: { select: { id: true, title: true } },
          },
        },
      },
    });
...
export async function confirmPayment(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const existing = await prisma.payment.findUnique({
      where: { id },
      include: { reservation: { include: { boarding: true } } },
    });
