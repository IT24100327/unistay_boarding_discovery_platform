...
import { CreateVisitRequestInput, RejectVisitRequestInput } from '../validators/visitRequest.validators';
import { BoardingStatus, VisitRequestStatus } from '@prisma/client';
...
export async function approveVisitRequest(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (new Date() > existing.expiresAt) {
      await prisma.visitRequest.update({ where: { id }, data: { status: VisitRequestStatus.EXPIRED } });
...
export async function rejectVisitRequest(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const visitRequest = await prisma.visitRequest.update({
      where: { id },
      data: { status: VisitRequestStatus.REJECTED, rejectionReason: reason },
      select: visitRequestSelect(),
    });
