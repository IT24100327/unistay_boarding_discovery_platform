...
import { CreateReservationInput, RejectReservationInput } from '../validators/reservation.validators';
import { BoardingStatus, ReservationStatus } from '@prisma/client';
...
function reservationSelect() {
  return {
    ...
    boarding: { select: { id: true, title: true, slug: true, city: true, district: true } },
  } as const;
}
...
// GET /api/v1/reservations/my-requests  (student)
...
// GET /api/v1/reservations/my-boardings  (owner)
...
// PATCH /api/v1/reservations/:id/approve  (owner)
...
// PATCH /api/v1/reservations/:id/reject  (owner)
export async function rejectReservation(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const reservation = await prisma.reservation.update({
      where: { id },
      data: { status: ReservationStatus.REJECTED, rejectionReason: reason },
      select: reservationSelect(),
    });
...
// PATCH /api/v1/reservations/:id/complete  (owner)
